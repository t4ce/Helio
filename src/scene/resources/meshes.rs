...
impl super::super::Scene {
    ...
    ///
    /// # Performance
    /// - CPU cost: O(vertices + indices) payload handoff; handle allocation is amortized O(1)
    ...
    pub fn insert_mesh(&mut self, mesh: MeshUpload) -> MeshId {
        self.mesh_pool_mut().insert(mesh)
    }
    ...
    pub fn insert_dynamic_mesh(&mut self, mesh: MeshUpload) -> MeshId {
        self.mesh_pool_mut().insert_dynamic(mesh)
    }
    ...
    pub fn remove_mesh(&mut self, id: MeshId) -> Result<()> {
        ...
        self.detach_actors_for_target(crate::scene::actor::SceneActorId::Mesh(id));
        Ok(())
    }
