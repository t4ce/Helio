...
Every frame updates SceneDB's registered `PlanetFrameAuthority`, which owns stable
generation-bearing planet identities plus sparse direct GPU rows. Helio projects
that canonical snapshot into page lookup metadata during pass `prepare`; the pass
has no independently editable frame registry. Canonical `i64`/`f64` planet
coordinates never enter existing Helio object transforms or the existing camera
uniform as absolute values.

Publication keeps three epochs deliberately separate: the authority-instance epoch
invalidates derived state when a distinct in-memory authority replaces the source,
the allocation epoch rebuilds consumers that bind the direct GPU buffer, and the
content generation refreshes compact CPU projections. Buffer growth or device
recreation therefore does not masquerade as an authored planet replacement.

SceneDB publishes `PlanetFrameUniform`; the pass additionally receives the
versioned streaming messages below:
