...
Building the web version is its own lightweight tool in `Helio-Examples` rather than a shell script. Open the egui Examples Hub with `cargo run -p helio-web-tools` and select **Build and serve web demos**, or run `cargo run -p helio-web-tools --bin web` directly. The `--headless` variant builds without the UI, writes the finished site out, and exits with a failure code if any demo did not build. Under the hood it invokes `wasm-pack` per demo and writes each landing page plus a master index. Prerequisites are `wasm-pack`, the `wasm32-unknown-unknown` Rust target, and a wasm-capable `clang` for the C dependencies.
...
For a real, non-trivial example, `../Helio-Examples/helio-web-demos/examples-wasm/vhs_effects.wgsl` is a complete camcorder shader with chromatic aberration, YIQ chroma drift, tracking noise, a head-switching bar at the bottom of frame, grain, and flicker.
