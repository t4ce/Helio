...
impl RenderGraph {
    ...
    pub fn lock(&mut self, width: u32, height: u32) {
        ...
        // This is a useful graph-construction diagnostic, but graph construction is
        // also the normal resize path. Never dump the full graph there unless it
        // was explicitly requested: it turns a live resize into pages of output.
        if std::env::var_os("HELIO_DUMP_RENDER_GRAPH").is_some() {
