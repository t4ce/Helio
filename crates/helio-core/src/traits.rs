//! Core traits for helio-core modular renderer.
//!
//! This module defines the fundamental trait that powers helio-core's modular architecture:
//!
//! - [`RenderPass`] - Trait for implementing render/compute passes
//!
//! # Design Pattern: Trait-Based Modularity
//!
//! Each render pass is a separate crate that implements the object-safe
//! `RenderPass` interface. `RenderGraph` stores passes behind trait objects,
//! allowing:
//!
//! - **Independent pass crates**: Add or replace passes without changing the core
//! - **Runtime composition**: Select and inspect heterogeneous pass pipelines
//! - **Validated dependencies**: `RenderGraph` checks declared resource ordering
//!   when the graph is constructed/locked, rather than at compile time
//!
...
//!         // Access canonical and derived scene buffers through ctx.scene.
//!         pass.draw(0..3, 0..1);
//!
//!         Ok(())
//!     }
...
///
/// `RenderPass` is the fundamental building block of helio-core. Each pass is a self-contained
