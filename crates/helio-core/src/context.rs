...
//!
//! Contexts provide borrowed references to the resources published for one pass:
//!
//! - assembling a context does not clone scene buffers;
//! - lifetimes prevent a published buffer from being freed while borrowed;
//! - individual pass implementations may still allocate, grow resources, or
//!   synchronize their own state when invalidation requires it.
//!
//! # Lifecycle
//!
...
//!
//! ```text
//! caller: flush/publish SceneDB and Helio-derived data
//! RenderGraph::execute()
//! └── for each pass:
...
/// # Design Pattern: Zero-Copy Access
///
/// Scene inputs and targets are borrowed rather than copied. Constructing the
/// context does not clone scene buffers or allocate a scene snapshot, and
/// Rust's borrow checker keeps those publications alive. This is a property of
/// context assembly, not a blanket allocation guarantee for pass code.
///
/// # Profiling
///
...
pub struct PassContext<'a> {
    ...
    /// Borrowed canonical SceneDB publications and Helio-derived render buffers.
