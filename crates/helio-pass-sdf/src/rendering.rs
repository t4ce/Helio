...
use crate::{
    SdfPass, INITIAL_BVH_CAPACITY, INITIAL_EDIT_CAPACITY, MAX_BRICKS_PER_LEVEL,
};
...
const _: () = assert!(std::mem::size_of::<GpuClipConfig>() == 96, "GpuClipConfig must be 96 bytes");
...
const _: () = assert!(std::mem::size_of::<GpuScrollState>() == 144, "GpuScrollState must be 144 bytes");
...
impl SdfPass {
    ...
    pub fn with_grid(
        ...
        let classify_bg = Self::build_classify_bg_impl(
            device, &classify_bgl,
            &clip_config_buffer, &scroll_state_buffer, &dirty_flags_buffer,
            &bvh_nodes_buffer, &per_brick_hashes_buffer,
            &per_brick_edit_lists_buffer, &all_brick_indices_buffer,
            &dirty_bricks_buffer, &eval_indirect_buffer,
        );

        let eval_bgl = eval_pipeline.get_bind_group_layout(0);
        let eval_bgs = Self::build_eval_bgs_impl(
            device, &eval_bgl,
            &level_params_buffers, &edit_buffer, &atlas_buffers,
            &dirty_bricks_buffer, &per_brick_edit_lists_buffer, &terrain_params_buffer,
        );
    ...
    fn prepare(&mut self, ctx: &PrepareContext) -> HelioResult<()> {
        ...
        if needs_upload {
            ...
            if !gpu_edits.is_empty() {
                ctx.queue.write_buffer(&self.edit_buffer, 0, bytemuck::cast_slice(&gpu_edits));
            }

            let bounds: Vec<(glam::Vec3, f32)> =
                self.edit_list.edits().iter().map(SdfPass::sdf_edit_bounds).collect();
            ...
            ctx.queue.write_buffer(&self.terrain_params_buffer, 0, bytemuck::bytes_of(&terrain_gpu));

            let clip_cfg = self.build_clip_config(edit_count, bvh_node_count, self.terrain_config.as_ref());
            ctx.queue.write_buffer(&self.clip_config_buffer, 0, bytemuck::bytes_of(&clip_cfg));

            for level in 0..self.level_count {
                let params = self.build_level_params(level, edit_count);
                ctx.queue.write_buffer(&self.level_params_buffers[level as usize], 0, bytemuck::bytes_of(&params));
            }

            self.edit_generation = self.edit_generation.wrapping_add(1);
            let edit_gen_offset: u64 = 128;
            ctx.queue.write_buffer(&self.scroll_state_buffer, edit_gen_offset, bytemuck::bytes_of(&self.edit_generation));
        ...
        if self.bindings_dirty {
            ...
            self.classify_bg = Self::build_classify_bg_impl(
                ctx.device, &classify_bgl,
                &self.clip_config_buffer, &self.scroll_state_buffer, &self.dirty_flags_buffer,
                &self.bvh_nodes_buffer, &self.per_brick_hashes_buffer,
                &self.per_brick_edit_lists_buffer, &self.all_brick_indices_buffer,
                &self.dirty_bricks_buffer, &self.eval_indirect_buffer,
            );
            let eval_bgl = self.eval_pipeline.get_bind_group_layout(0);
            self.eval_bgs = Self::build_eval_bgs_impl(
                ctx.device, &eval_bgl,
                &self.level_params_buffers, &self.edit_buffer, &self.atlas_buffers,
                &self.dirty_bricks_buffer, &self.per_brick_edit_lists_buffer, &self.terrain_params_buffer,
            );
            self.march_bg = None;
    ...
    fn execute(&mut self, ctx: &mut PassContext) -> HelioResult<()> {
        ...
        if self.scroll_bg_camera_key != camera_ptr || self.scroll_bg.is_none() {
            ...
            self.scroll_bg = Some(Self::build_scroll_bg(
                ctx.device, &scroll_bgl,
                ctx.scene.camera, &self.clip_config_buffer,
                &self.scroll_state_buffer, &self.dirty_flags_buffer,
            ));
        ...
        if self.march_bg_camera_key != camera_ptr || self.march_bg.is_none() {
            self.march_bg = Some(Self::build_march_bg(
                ctx.device, &self.march_bgl,
                ctx.scene.camera, &self.clip_config_buffer,
                &self.scroll_state_buffer, &self.atlas_buffers,
        ...
        if !self.gpu_passes_clean {
            unsafe { &mut *ctx.encoder_ptr }.copy_buffer_to_buffer(
                &self.eval_indirect_template_buffer, 0,
                &self.eval_indirect_buffer, 0,
...

            {
                ...
                for (level, eval_bg) in self.eval_bgs.iter().enumerate() {
                    ...
                    cpass.dispatch_workgroups_indirect(&self.eval_indirect_buffer, (level * 3 * 4) as u64);
                }
            }
        }
