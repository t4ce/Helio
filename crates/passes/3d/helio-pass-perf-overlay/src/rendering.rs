...
const MAX_PROFILE_LIGHTS: usize = 32;

/// Pass-owned synthetic input for the material timing microbenchmark.
///
/// This intentionally does not alias the scene's canonical light rows: the
/// profiler's compact 48-byte test ABI and fixed 32-light workload are not
/// authored scene data.
#[repr(C)]
#[derive(Clone, Copy, bytemuck::Pod, bytemuck::Zeroable)]
struct SyntheticProfileLight {
    position: [f32; 4],
    color: [f32; 4],
    direction: [f32; 4],
}
...
impl MaterialProfiler {
    pub fn new(device: &wgpu::Device, queue: &wgpu::Queue) -> Self {
        ...
        let synthetic_lights = std::array::from_fn::<_, MAX_PROFILE_LIGHTS, _>(|index| {
            let angle = index as f32 * std::f32::consts::TAU / MAX_PROFILE_LIGHTS as f32;
            SyntheticProfileLight {
                position: [angle.cos() * 3.0, angle.sin() * 3.0, 2.0, 0.0],
                color: [0.75 + 0.25 * angle.cos(), 0.75 + 0.25 * angle.sin(), 1.0, 4.0],
                direction: [0.0, 0.0, -1.0, 0.0],
            }
        });
        let synthetic_lights_buf = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("Material Profile Synthetic Lights"),
            size: std::mem::size_of_val(&synthetic_lights) as u64,
            usage: wgpu::BufferUsages::STORAGE,
            mapped_at_creation: true,
        });
        synthetic_lights_buf
            .slice(..)
            .get_mapped_range_mut()
            .expect("synthetic material profile lights should be mapped")
            .copy_from_slice(bytemuck::cast_slice(&synthetic_lights));
        synthetic_lights_buf.unmap();
    ...
    pub fn profile_next(
        ...
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry {
                    binding: 1,
                    resource: self.synthetic_lights.as_entire_binding(),
                },
            ],
        });
