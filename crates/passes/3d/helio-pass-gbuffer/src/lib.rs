...
pub struct GBufferPass {
    ...
    /// Content epoch as of the last sync. This catches replacement under an
    /// existing class id without allocating a key vector every frame.
    last_template_epoch: u64,
    /// Content epoch of graph-generated WGSL. Graph keys may be deliberately
    /// reused during live editing, so the shader cache cannot key on hash alone.
    last_graph_wgsl_epoch: u64,
    pipeline_layout: wgpu::PipelineLayout,
    bind_group_layout_0: wgpu::BindGroupLayout,
    bind_group_layout_1: wgpu::BindGroupLayout,
    /// Group 0: camera + canonical object rows + temporal history.
        ...
        let bind_group_layout_0 =
            device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                ...
                entries: &[
                    ...
                    // binding 2: canonical object spatial rows
                    wgpu::BindGroupLayoutEntry {
                        binding: 2,
                    ...
                    // binding 7: canonical object render-reference rows
                    wgpu::BindGroupLayoutEntry {
                        binding: 7,
                        visibility: wgpu::ShaderStages::VERTEX,
                        ty: wgpu::BindingType::Buffer {
                    ...
                    // binding 8: Helio previous-frame object history
                    wgpu::BindGroupLayoutEntry {
                        binding: 8,
                        visibility: wgpu::ShaderStages::VERTEX,
                        ty: wgpu::BindingType::Buffer {
...
impl RenderPass for GBufferPass {
    ...
    fn execute(&mut self, ctx: &mut PassContext) -> HelioResult<()> {
        ...
        // Rebuild bind group 0 whenever a canonical/history allocation changes.
        let camera_ptr = ctx.scene.camera as *const _ as usize;
        let spatial_ptr = ctx.scene.object_spatial as *const _ as usize;
        let render_ptr = ctx.scene.object_render as *const _ as usize;
        ...
        let coordinate_spaces_prev_ptr = ctx.scene.coordinate_spaces_prev as *const _ as usize;
        let key = (
            camera_ptr,
            spatial_ptr,
            render_ptr,
        ...
        // The view/sampler array and both SceneDB row buffers grow
        // independently, so all three allocation epochs are load-bearing.
        let binding_key = ctx
            .scene
        ...
        if let Some(reg_any) = ctx.scene.template_registry.as_ref() {
            if let Some(shared) = reg_any.downcast_ref::<helio::radiant::SharedTemplateRegistry>() {
                ...
                if self.last_template_epoch != epoch {
                    // Registry changed — pipelines must be re-created
                    self.pipelines.clear();
                    self.shader_cache = helio::radiant::RadiantShaderCache::new();
                    self.last_template_epoch = epoch;
                }
