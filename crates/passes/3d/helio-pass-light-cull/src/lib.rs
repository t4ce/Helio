...
pub struct LightCullPass {
    ...
    /// Key: canonical lights plus compact projection allocation identities.
...
impl RenderPass for LightCullPass {
    ...
    fn execute(&mut self, ctx: &mut PassContext) -> HelioResult<()> {
        ...
        let projections_ptr = ctx.scene.light_projections as *const _ as usize;
        let key = (camera_ptr, lights_ptr, projections_ptr);
