...
use crate::{
    SdfPass, SdfPassConfigError, INITIAL_BVH_CAPACITY, MAX_BRICKS_PER_LEVEL,
};
...
pub(crate) struct GpuClipConfig {
    ...
    pub atlas_words_per_level: u32,
    pub canonical_order_scan: u32,
    pub _pad2: u32,
    pub _pad3: u32,
...
impl SdfPass {
    pub fn new(
        ...
        .expect("the default SDF clipmap configuration is valid")
    }
    ...
    pub fn with_grid(
        ...
        volume_min: [f32; 3],
        volume_max: [f32; 3],
    ) -> Result<Self, SdfPassConfigError> {
        let level_count = DEFAULT_CLIP_LEVELS;
        let brick_size = DEFAULT_BRICK_SIZE;
        if grid_dim == 0 || grid_dim % brick_size != 0 {
            return Err(SdfPassConfigError::GridNotBrickAligned);
        }
        if !volume_min.into_iter().all(f32::is_finite)
            || !volume_max.into_iter().all(f32::is_finite)
            || !(0..3).all(|axis| volume_max[axis] > volume_min[axis])
        {
            return Err(SdfPassConfigError::InvalidVolumeBounds);
        }
        let brick_grid_dim = grid_dim / brick_size;
        let bricks_per_level = brick_grid_dim
            .checked_mul(brick_grid_dim)
            .and_then(|square| square.checked_mul(brick_grid_dim))
            .ok_or(SdfPassConfigError::BrickCapacityExceeded {
                requested: u32::MAX,
                maximum: MAX_BRICKS_PER_LEVEL,
            })?;
        if bricks_per_level > MAX_BRICKS_PER_LEVEL {
            return Err(SdfPassConfigError::BrickCapacityExceeded {
                requested: bricks_per_level,
                maximum: MAX_BRICKS_PER_LEVEL,
            });
        }
        ...
        let atlas_word_count = (u64::from(bricks_per_level)
            * u64::from(padded_brick_voxels))
            .div_ceil(4);
        let atlas_data_bytes = atlas_word_count * 4;
        let atlas_alignment = u64::from(device.limits().min_storage_buffer_offset_alignment);
        let atlas_level_byte_stride = atlas_data_bytes.div_ceil(atlas_alignment) * atlas_alignment;
        let atlas_words_per_level = u32::try_from(atlas_level_byte_stride / 4)
            .map_err(|_| SdfPassConfigError::DeviceStorageLimit)?;
        let atlas_total_bytes = atlas_level_byte_stride * u64::from(level_count);
        ...
        let storage_sizes = [
            atlas_total_bytes,
            (INITIAL_BVH_CAPACITY * std::mem::size_of::<GpuBvhNode>()) as u64,
            u64::from(level_count) * u64::from(bricks_per_level) * 4,
            u64::from(level_count)
                * u64::from(bricks_per_level)
                * u64::from(EDIT_LIST_STRIDE)
                * 4,
            u64::from(level_count)
                * u64::from(MAX_BRICKS_PER_LEVEL)
                * std::mem::size_of::<[i32; 4]>() as u64,
        ];
        if storage_sizes.into_iter().any(|size| size > storage_limit) {
            return Err(SdfPassConfigError::DeviceStorageLimit);
        }
        let atlas_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("SDF Packed Level Atlases"),
            size: atlas_total_bytes,
        ...
        // Evaluation bind groups are created during prepare once SceneDB has
        // published the direct authored edit and terrain allocations.
        let eval_bgs = Vec::new();
        ...
        Ok(Self {
            ...
            atlas_buffer,
            atlas_level_byte_stride,
            atlas_words_per_level,
            ...
            last_gen: u64::MAX,
            bound_edit_epoch: None,
            bound_terrain_epoch: None,
            edit_generation: 0,
            bindings_dirty: true,
            debug_mode: false,
            enabled: true,
    ...
    fn build_eval_bgs_impl(
        ...
        atlas_buf: &wgpu::Buffer,
        atlas_level_byte_stride: u64,
        dirty_bricks_buf: &wgpu::Buffer,
        per_brick_edit_lists_buf: &wgpu::Buffer,
                    ...
                    entries: &[
                        ...
                        wgpu::BindGroupEntry {
                            ...
                            resource: wgpu::BindingResource::Buffer(wgpu::BufferBinding {
                                buffer: atlas_buf,
                                offset: i as u64 * atlas_level_byte_stride,
                                size: std::num::NonZeroU64::new(atlas_level_byte_stride),
                            }),
                        },
                        wgpu::BindGroupEntry {
                            binding: 3,
...
impl RenderPass for SdfPass {
    ...
    fn prepare(&mut self, ctx: &PrepareContext) -> HelioResult<()> {
        ...
        let Some(edit_epoch) = ctx.scene.sdf_edit_buffer_epoch() else {
            self.authority_bound = false;
            self.gpu_passes_clean = true;
            return Ok(());
        };
        let Some(terrain_epoch) = ctx.scene.sdf_terrain_buffer_epoch() else {
            self.authority_bound = false;
            self.gpu_passes_clean = true;
            return Ok(());
        };
        self.authority_bound = true;
        if self.bound_edit_epoch != Some(edit_epoch)
            || self.bound_terrain_epoch != Some(terrain_epoch)
        {
            self.bindings_dirty = true;
        }

        let gen = ctx.scene.sdf_content_generation();
        let needs_upload = gen != self.last_gen;

        if needs_upload {
            let edit_count = ctx.scene.sdf_edit_count();
            debug_assert_eq!(ctx.scene.sdf_edit_bounds().len(), edit_count as usize);
            let bvh = build_flat_bvh(ctx.scene.sdf_edit_bounds());
            ...
            let clip_cfg = self.build_clip_config(
                edit_count,
                bvh_node_count,
                ctx.scene.sdf_terrain_y_bounds(),
                ctx.scene.sdf_requires_canonical_scan(),
            );
            ctx.queue
        ...
        if self.bindings_dirty {
            ...
            self.eval_bgs = Self::build_eval_bgs_impl(
                ...
                ctx.scene.sdf_edit_buffer(),
                &self.atlas_buffer,
                self.atlas_level_byte_stride,
                &self.dirty_bricks_buffer,
                &self.per_brick_edit_lists_buffer,
                ctx.scene.sdf_terrain_buffer(),
            );
            self.bound_edit_epoch = Some(edit_epoch);
            self.bound_terrain_epoch = Some(terrain_epoch);
            self.march_bg = None;
    ...
    fn execute(&mut self, ctx: &mut PassContext) -> HelioResult<()> {
        if !self.enabled || !self.authority_bound {
            return Ok(());
        }
        ...
        if !self.gpu_passes_clean {
            ...
            {
                let wgs_x = self.bricks_per_level.div_ceil(64);
                let mut cpass = ctx.begin_compute_pass(&wgpu::ComputePassDescriptor {
                    label: Some("SDF Classify"),
                    timestamp_writes: None,
                });
