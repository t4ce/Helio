use helio_pass_cloud_volume::{RenderParams, SimParams, VOLUME_SIZE, RENDER_PARAMS_SIZE, SIM_PARAMS_SIZE};

#[test]
fn authored_contract_sizes_and_volume_are_exact() {
    assert_eq!(std::mem::size_of::<SimParams>(), SIM_PARAMS_SIZE as usize);
    assert_eq!(std::mem::size_of::<RenderParams>(), RENDER_PARAMS_SIZE as usize);
