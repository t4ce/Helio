...
//!   4. ScanBlocks   — sequential cumulative sum per emitter; writes alive counts
...
const MAX_EMITTERS: u32 = libhelio::CORONA_MAX_EMITTERS;
const WG: u32 = 256;
...
pub struct CoronaPass {
    ...
    /// Two fixed regions in one binding: alive counts followed by persistent
    /// spawn cursors. Sharing the existing storage binding keeps Corona at its
    /// current nine-storage-buffer compute tier.
    emitter_state_buf: wgpu::Buffer,
    ...
    uploaded_reset_epoch: u64,
    reset_simulation: bool,
    max_particles: u32,
    emitter_count: u32,
    max_sort_steps: u32, // capacity of sort_steps_buf in step count

    // CPU-normalized authored definitions and non-overlapping offsets. Runtime
    // spawn cursors are exclusively GPU simulation state.
...
impl CoronaPass {
    pub fn new(
        ...
        let emitter_state_buf = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("Corona Emitter State"),
            size: MAX_EMITTERS as u64 * 2 * std::mem::size_of::<u32>() as u64,
    ...
    fn create_bgl(device: &wgpu::Device, for_render: bool) -> wgpu::BindGroupLayout {
        ...
        device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            ...
            entries: &[
                ...
                // Authored definitions are immutable to simulation. Runtime
                // spawn cursors live in binding 4 instead.
                Self::storage_entry(2, storage_visibility, true),
    ...
    fn build_bg(
        ...
        device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry {
                    binding: 4,
                    resource: emitter_state.as_entire_binding(),
                },
                wgpu::BindGroupEntry {
                    binding: 5,
    ...
    fn clear_buffers(encoder: &mut wgpu::CommandEncoder, buffers: &[&wgpu::Buffer]) {
...
impl RenderPass for CoronaPass {
    ...
    fn prepare(&mut self, ctx: &PrepareContext) -> HelioResult<()> {
        if let Some(data) = ctx.frame_resources.corona_emitters.get() {
            if data.reset_epoch != self.uploaded_reset_epoch {
                self.uploaded_reset_epoch = data.reset_epoch;
            ...
            if data.generation != self.uploaded_generation {
                ...
                for (i, src_em) in src.iter().enumerate() {
                    ...
                    self.cpu_emitters[i] = *src_em;
                    self.cpu_emitters[i].particle_count = aligned_count;
                    self.cpu_emitters[i].particle_offset = offset;
                    // This authored ABI field is intentionally inert. The
                    // actual cursor lives in emitter_state_buf and is advanced
                    // only by cs_emit, so descriptor uploads cannot reset it.
                    self.cpu_emitters[i].spawn_cursor = 0;

                    offset = offset.saturating_add(aligned_count);
                }

                let bytes: &[u8] = bytemuck::cast_slice(&self.cpu_emitters);
                let cap = (MAX_EMITTERS as usize * em_size).min(bytes.len());
                ctx.write_buffer(&self.emitter_buf, 0, &bytes[..cap]);

                // Slots past the new authored count have ended their lifetime.
                // Clear only their GPU cursors so a later bulk-list growth
                // starts genuinely new positional emitters at cursor zero.
                let previous_count = self.emitter_count as usize;
                if count < previous_count {
                    let zeros = [0u32; MAX_EMITTERS as usize];
                    let retired = previous_count - count;
                    let cursor_offset = (MAX_EMITTERS as usize + count)
                        * std::mem::size_of::<u32>();
                    ctx.write_buffer(
                        &self.emitter_state_buf,
                        cursor_offset as u64,
                        bytemuck::cast_slice(&zeros[..retired]),
                    );
                }
            ...
            if self.emitter_count != 0 {
                let zeros = [0u32; MAX_EMITTERS as usize];
                let cursor_offset =
                    MAX_EMITTERS as u64 * std::mem::size_of::<u32>() as u64;
                ctx.write_buffer(
                    &self.emitter_state_buf,
                    cursor_offset,
                    bytemuck::cast_slice(&zeros[..self.emitter_count as usize]),
                );
            }
...
mod tests {
    ...
    use super::{CoronaPass, CoronaUniforms, MAX_EMITTERS};

    const COMPUTE_STORAGE_BINDINGS: u32 = 9;

    async fn cursors_after_edit_and_reset() -> Option<(u32, u32)> {
        ...
        let adapter = adapter?;
        if adapter.limits().max_storage_buffers_per_shader_stage < COMPUTE_STORAGE_BINDINGS {
            eprintln!(
                "skipping Corona cursor regression: adapter lacks the existing nine-storage tier"
            );
            return None;
        }

        let mut limits = wgpu::Limits::downlevel_defaults();
        limits.max_storage_buffers_per_shader_stage = COMPUTE_STORAGE_BINDINGS;
        let (device, queue) = adapter
            .request_device(&wgpu::DeviceDescriptor {
                label: Some("Corona Cursor Regression Device"),
                required_features: wgpu::Features::empty(),
                required_limits: limits,
                ..Default::default()
            })
            .await
            .expect("adapter must support Corona's existing binding tier");
        device.on_uncaptured_error(Arc::new(|error| {
            panic!("Corona cursor regression GPU validation error: {error:?}");
        }));

        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Corona Cursor Regression Shader"),
            source: wgpu::ShaderSource::Wgsl(
                include_str!("../shaders/corona.wgsl").into(),
            ),
        });
        let bgl = CoronaPass::create_bgl(&device, false);
        let layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("Corona Cursor Regression Layout"),
            bind_group_layouts: &[Some(&bgl)],
            immediate_size: 0,
        });
        let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("Corona Cursor Regression Pipeline"),
            layout: Some(&layout),
            module: &shader,
            entry_point: Some("cs_emit"),
            compilation_options: Default::default(),
            cache: None,
        });
        ...
        let uniforms = buffer(
            "Corona Cursor Regression Uniforms",
            std::mem::size_of::<CoronaUniforms>() as u64,
            wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
        );
        let particles = buffer(
            "Corona Cursor Regression Particles",
            16 * std::mem::size_of::<libhelio::GpuCoronaParticle>() as u64,
            wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST,
        );
        let emitters = buffer(
            "Corona Cursor Regression Emitters",
            std::mem::size_of::<libhelio::GpuCoronaEmitter>() as u64,
            wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST,
        );
        let compact = buffer(
            "Corona Cursor Regression Compact",
            4,
            wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST,
        );
        let emitter_state = buffer(
            "Corona Cursor Regression Emitter State",
            MAX_EMITTERS as u64 * 2 * std::mem::size_of::<u32>() as u64,
            wgpu::BufferUsages::STORAGE
                | wgpu::BufferUsages::COPY_SRC
                | wgpu::BufferUsages::COPY_DST,
        );
        let draw_staging = buffer(
            "Corona Cursor Regression Draw Staging",
            std::mem::size_of::<libhelio::GpuCoronaDrawIndirect>() as u64,
            wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST,
        );
        let camera = buffer(
            "Corona Cursor Regression Camera",
            1024,
            wgpu::BufferUsages::STORAGE,
        );
        let prefix = buffer(
            "Corona Cursor Regression Prefix",
            4,
            wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST,
        );
        let block_sums = buffer(
            "Corona Cursor Regression Block Sums",
            4,
            wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST,
        );
        let sort_keys = buffer(
            "Corona Cursor Regression Sort Keys",
            4,
            wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST,
        );
        let texture = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("Corona Cursor Regression Texture"),
            size: wgpu::Extent3d {
                width: 1,
                height: 1,
                depth_or_array_layers: 1,
        ...
        let bind_group = CoronaPass::build_bg(
            &device,
            &bgl,
            &uniforms,
            &particles,
        ...
        queue.write_buffer(&uniforms, 0, bytemuck::bytes_of(&frame));
        let mut emitter = libhelio::GpuCoronaEmitter::zeroed();
        emitter.emit_params[0] = 3.0;
        emitter.emit_params[1] = 1.0;
        emitter.extras[3] = 1.0;
        emitter.particle_count = 16;
        queue.write_buffer(&emitters, 0, bytemuck::bytes_of(&emitter));

        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("Corona Cursor First Emit"),
        });
        {
        ...
        // Edit authored data after simulation has advanced. Even a hostile
        // value in the legacy ABI field must not replace GPU cursor state.
        emitter.transform[3][0] = 42.0;
        emitter.emit_params[0] = 2.0;
        emitter.spawn_cursor = 13;
        queue.write_buffer(&emitters, 0, bytemuck::bytes_of(&emitter));

        let readback = buffer(
            "Corona Cursor Regression Readback",
            8,
            wgpu::BufferUsages::COPY_DST | wgpu::BufferUsages::MAP_READ,
        );
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("Corona Cursor Second Emit"),
        });
        {
            let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("Corona Cursor Second Emit"),
        ...
        let cursor_offset = MAX_EMITTERS as u64 * std::mem::size_of::<u32>() as u64;
        encoder.copy_buffer_to_buffer(&emitter_state, cursor_offset, &readback, 0, 4);
        queue.submit(Some(encoder.finish()));

        // Canonical clear followed by re-add before any empty frame is a
        // distinct lifetime boundary. The pass clears simulation state at the
        // start of the next execution, then emits from cursor zero.
        emitter.transform[3][0] = 84.0;
        emitter.emit_params[0] = 4.0;
        emitter.spawn_cursor = 11;
        queue.write_buffer(&emitters, 0, bytemuck::bytes_of(&emitter));
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("Corona Cursor Reset And Re-add"),
        });
        CoronaPass::clear_buffers(
            &mut encoder,
            &[
                &particles,
                &compact,
        ...
        {
            let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("Corona Cursor Emit After Reset"),
                timestamp_writes: None,
            });
            pass.set_pipeline(&pipeline);
            pass.set_bind_group(0, &bind_group, &[]);
        ...
        encoder.copy_buffer_to_buffer(&emitter_state, cursor_offset, &readback, 4, 4);
        queue.submit(Some(encoder.finish()));

        let slice = readback.slice(..);
        let (sender, receiver) = std::sync::mpsc::channel();
        slice.map_async(wgpu::MapMode::Read, move |result| {
            sender.send(result).expect("send Corona cursor map result");
        });
        device
            .poll(wgpu::PollType::wait_indefinitely())
            .expect("poll Corona cursor readback");
        receiver
            .recv()
            .expect("receive Corona cursor map result")
            .expect("map Corona cursor readback");
        let mapped = slice
            .get_mapped_range()
            .expect("read mapped Corona cursor bytes");
        let cursor_after_edit = u32::from_ne_bytes(mapped[..4].try_into().unwrap());
        let cursor_after_reset = u32::from_ne_bytes(mapped[4..8].try_into().unwrap());
        drop(mapped);
        readback.unmap();
        Some((cursor_after_edit, cursor_after_reset))
    }

    #[test]
    fn authored_edit_preserves_cursor_and_clear_readd_resets_without_empty_frame() {
        let Some((cursor_after_edit, cursor_after_reset)) =
            pollster::block_on(cursors_after_edit_and_reset())
        else {
            eprintln!("skipping Corona cursor regression: no GPU adapter available");
            return;
        };
        assert_eq!(
            cursor_after_edit, 5,
            "three spawns before the edit plus two after it"
        );
        assert_eq!(
            cursor_after_reset, 4,
            "clear and immediate re-add must start a new GPU cursor lifetime"
        );
    }
}
