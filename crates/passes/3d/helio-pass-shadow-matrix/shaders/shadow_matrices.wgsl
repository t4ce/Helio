...
/// Must match GpuLight in libhelio/src/light.rs / SceneLightRow (128 bytes)
struct GpuLight {
...
fn compute_point_light_matrices(light_idx: u32, position: vec3f, range: f32) {
    let base = projected_light(light_idx).shadow_index;
...
fn compute_directional_cascades(light_idx: u32, direction: vec3f) {
    ...
    for (var i = 0u; i < 8u; i++) {
        // Camera projection carries temporal NDC jitter. Feed the matching
        // shifted clip coordinate through its inverse so CSM geometry is built
        // from the stable, unjittered frustum. This also keeps the GPU matrix
        // hash aligned with Scene's CPU submission gate.
        let stable_clip = vec4f(
            ndc[i].xy + cameras[0].jitter_frame.xy,
            ndc[i].z,
            ndc[i].w,
        );
        let v = cameras[0].inv_view_proj * stable_clip;
        world[i] = v.xyz / v.w;
    }
