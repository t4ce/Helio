...
//!
//! Runs AFTER ShadowDirtyPass and BEFORE ShadowPass. For each movable mesh batch,
//! tests every member's bounding sphere against each dirty shadow face's
//! frustum (planes extracted from the VP matrix via Gribb-Hartmann). If any
//! member intersects, the complete batch is conservatively appended. This
//! keeps instancing while avoiding false-negative shadows from using only the
//! batch's first, spatially unrelated member.
//!
//! # Buffers produced
//!
//! | Buffer              | Format                                          |
...
//!
//! ShadowPass reads these buffers to issue per-face indirect draws. The global
//! `shadow_movable_indirect` list remains the authoritative correctness fallback.
//!
//! # Integration
//!
//! ```ignore
...
//!     device, queue, face_dirty_buf, face_indirect_buf, face_counts_buf,
//!     atlas_size, atlas_layers,
//! );
//! ```
    ...
    pub fn new(
        ...
        let bgl = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            ...
            entries: &[
                ...
                // 2: canonical object spatial rows (storage read)
                wgpu::BindGroupLayoutEntry {
                    binding: 2,
                    visibility: wgpu::ShaderStages::COMPUTE,
                    ty: wgpu::BindingType::Buffer {
                ...
                // 8: movable shadow slot -> canonical SceneDB row
                wgpu::BindGroupLayoutEntry {
                    binding: 8,
                    visibility: wgpu::ShaderStages::COMPUTE,
                    ty: wgpu::BindingType::Buffer {
...
impl RenderPass for ShadowCullPass {
    ...
    fn execute(&mut self, ctx: &mut PassContext) -> HelioResult<()> {
        ...
        // ShadowPass uses the exact authoritative list when the fixed
        // compacted capacity is exceeded. Avoid the expensive all-face scan,
        // and leave explicit zero counts rather than a stale prior frame.
        if movable_count == 0 || movable_count > MAX_DRAWS_PER_FACE {
            return Ok(());
        }
        ...
        let key = (sm_ptr, spatial_ptr, src_ptr, fd_ptr, cs_ptr, source_ptr);
