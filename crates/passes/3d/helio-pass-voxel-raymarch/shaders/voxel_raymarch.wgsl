...
// GpuLight (128 bytes, matches libhelio::GpuLight — see deferred_lighting.wgsl).
struct GpuLight {
...
@group(0) @binding(9) var<storage, read> voxel_volume_indices: array<u32>;
@group(0) @binding(10) var<storage, read> palettes: array<GpuVoxelMaterial>;
...
fn trace_volume(ro: vec3<f32>, rd: vec3<f32>, vol: GpuVoxelVolume) -> HitResult {
    ...
    if t_entry > t_exit || t_exit < 0.0 {
        return HitResult(0u, 0u, 0u, 0u, vec3<f32>(0.0), vec3<f32>(0.0));
    }
    ...
    for (var i = 0u; i < max_steps; i++) {
        ...
        // Each canonical volume owns a stable SceneDB residency region. Local
        // brick coordinates are relative to that region, never to buffer row 0.
        let local_brick_idx = brick_z * i32(vol.brick_grid_dim) * i32(vol.brick_grid_dim)
                            + brick_y * i32(vol.brick_grid_dim)
                            + brick_x;

        if local_brick_idx >= 0 {
            let brick_idx = vol.brick_offset + u32(local_brick_idx);
            let meta_word = brick_pool[brick_idx * 2u];
...
fn main(
    ...
    var best_hit = HitResult(0u, 0u, 0u, 0u, vec3<f32>(0.0), vec3<f32>(0.0));
    var best_t = 1e10;

    for (var vi = 0u; vi < params.volume_count; vi++) {
        let vol = volumes[voxel_volume_indices[vi]];
        ...
        if h.hit > 0u {
            let world_position = (vol.local_to_world * vec4<f32>(h.position, 1.0)).xyz;
            let world_normal = normalize((transpose(w2l) * vec4<f32>(h.normal, 0.0)).xyz);
            let hit_t = length(world_position - ro);
            if hit_t < best_t {
                best_t = hit_t;
                best_hit = HitResult(
                    h.hit,
    ...
    if best_hit.hit > 0u {
        ...
        if best_hit.material < best_hit.palette_count {
            material = palettes[best_hit.palette_offset + best_hit.material];
        }
        ...
        for (var li = 0u; li < params.light_count; li++) {
            direct += light_contribution(projected_light(li), best_hit.position, n);
        }
