    ...
    fn rebuild_compute_bg(&mut self, ctx: &PassContext) {
        let bg = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry { binding: 8, resource: ctx.scene.light_projections.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 9, resource: ctx.scene.voxel_volume_indices.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 10, resource: ctx.scene.voxel_palette_pool.as_entire_binding() },
            ],
        });
        ...
        self.compute_bg_key = Some((
            ...
            ctx.scene.voxel_volume_epoch.unwrap_or(0),
            ctx.scene.voxel_volume_indices as *const _ as usize,
            ctx.scene.voxel_brick_pool as *const _ as usize,
            ctx.scene.voxel_data_pool as *const _ as usize,
            ctx.scene.voxel_palette_pool as *const _ as usize,
            ctx.scene.voxel_residency_epoch.unwrap_or(0),
        ));
    }
...
impl RenderPass for VoxelRayMarchPass {
    ...
    fn prepare(&mut self, ctx: &PrepareContext) -> HelioResult<()> {
        let voxel_volume_count = ctx.scene.voxel_volume_indices.len() as u32;
        if voxel_volume_count != self.last_volume_count
    ...
    fn execute(&mut self, ctx: &mut PassContext) -> HelioResult<()> {
        ...
        if self.compute_bg_key != Some(key) || self.compute_bg.is_none() {
