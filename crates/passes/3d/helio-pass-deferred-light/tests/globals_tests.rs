...
#[test]
fn water_tail_row_fills_16_bytes() {
    // Row 7: reflection toggles + water count + explicit pad = 16 bytes.
    let row7 = 4 * size_of::<u32>();
    assert_eq!(row7, 16);
}

#[test]
fn globals_112_bytes_leaves_no_trailing_gap_in_16_byte_rows() {
    assert_eq!(
        112 % 16,
        0,
        "112 bytes must pack cleanly into 16-byte WGSL rows"
    );
}
