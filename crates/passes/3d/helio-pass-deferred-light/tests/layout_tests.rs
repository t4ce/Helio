...
//   has_rc_gi: u32,          // 4
//   num_tiles_x: u32,        // 4
//   reflection_count: u32,   // 4  → row-6 = 16 bytes
//   enable_reflections: u32, // 4
//   enable_env: u32,         // 4
//   water_count: u32,        // 4
//   _water_padding: u32,     // 4  → row-7 = 16 bytes
// }                          // Total = 112 bytes
...
#[test]
fn deferred_globals_each_scalar_tail_row_is_16_bytes() {
    let last_row = 4 * size_of::<u32>();
    assert_eq!(last_row, 16);
}

#[test]
fn deferred_globals_total_size_is_112_bytes() {
    let row1 = size_of::<u32>() * 2 + size_of::<f32>() * 2; // 16
    let vec4_rows = 4 * (4 * size_of::<f32>()); // 64
    let scalar_tail_rows = 2 * 4 * size_of::<u32>(); // 32
    assert_eq!(row1 + vec4_rows + scalar_tail_rows, 112);
}
...
#[test]
fn deferred_globals_total_divided_by_16_is_seven_rows() {
    assert_eq!(112 / 16, 7);
}
...
#[test]
fn globals_buf_size_112_satisfies_min_uniform_binding_size_multiple() {
